# -*- coding: utf-8 -*-
"""SAM Post-Training Quantization Pipeline."""
...
def ptq(
    model: SamModelStruct | torch.nn.Module,
    config: SamQuantConfig,
    ...
    """Post-training quantization of a SAM model.

    Args:
        model (`SamModelStruct` or `torch.nn.Module`):
            The SAM model to quantize.
        config (`SamQuantConfig`):
    ...
    Returns:
        `SamModelStruct`: The quantized SAM model.
    """
    logger = tools.logging.getLogger(__name__)
    logger.info("=" * 80)
    logger.info("SAM Post-Training Quantization")
    logger.info("=" * 80)

    # Convert to SamModelStruct if needed
    if not isinstance(model, SamModelStruct):
        logger.info("Constructing SAM model structure...")
        model = SamModelStruct.construct(model)
    assert isinstance(model, SamModelStruct)
    ...
    if quant_acts:
        ...
        for key, module_name, module, parent_struct, field_name in model.named_key_modules():
            ...
            if quant_ipts:
                ipt_quantizer = SamActivationQuantizer(
                    config=config.ipts,
                    key=f"{key}.input",
                )
            ...
            if quant_opts:
                opt_quantizer = SamActivationQuantizer(
                    config=config.opts,
                    key=f"{key}.output",
                )
